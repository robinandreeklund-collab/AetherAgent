...
/// role+direction+cluster key. Strategy is derived via regret matching:
///   weight = heuristic * (1 + positive_signal)
/// where positive_signal = cum_positive / (cum_positive + |cum_negative| + 1)
///
/// DCFR advantage over Beta-distribution:
/// - Linear discounting: older regrets decay → faster adaptation to page changes
/// - Regret matching: provably converges to Nash equilibrium in O(1/√T)
/// - Handles cold-start better: heuristic prior used until data accumulates
///
...
fn learned_weight_precomputed(
    ...
    // Signal strength: how much positive regret dominates
    // Range: 0.0 (all negative) to 1.0 (all positive)
    let signal = pos / (total + 1.0);
    // Scale: heuristic * (0.5 when negative, up to 1.8 when strongly positive)
...
impl ResonanceField {
    ...
    pub fn feedback(&mut self, goal: &str, successful_node_ids: &[u32]) {
        ...
        // For each edge, compute regret = utility(action) - utility(default).
        // Success → positive regret for that direction (it was worth propagating)
        // Failure → negative regret (propagation didn't help)
        ...
        for (child_id, _parent_id, child_role, parent_role) in &edges {
            ...
            // DCFR regret: success → positive cumulative regret, failure → negative
            // Regret magnitude scales with confidence (amplitude)
            let regret = if is_success {
                conf.max(0.1) // At least 0.1 positive regret for success
            } else {
                -(1.0 - conf).max(0.1) // Negative regret for failure
            };
            ...
            if regret > 0.0 {
                de.0 += regret; // Accumulate positive regret
            } else {
                de.1 += regret; // Accumulate negative regret (stored as negative)
            }
